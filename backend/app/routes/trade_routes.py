...
def open_trade(payload: schemas.TradeOpenRequest, db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
    try:
        trade = trading_service.open_trade(db, user_id, payload.direction, payload.amount, payload.duration_seconds)
...
def trade_history(db: Session = Depends(get_db), user_id: str = Depends(get_current_user_id)):
    return (
        db.query(models.Trade)
        .filter(models.Trade.user_id == user_id, models.Trade.status != models.TradeStatus.OPEN)
        .order_by(models.Trade.settled_at.desc()).limit(100).all()
    )
